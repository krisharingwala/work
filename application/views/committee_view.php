  ...
  <body>
                ...
                <div class="col-lg-8 col-md-12 d-flex justify-content-end">
                  <a href="#" class="btn btn-teal-info btn-with-icon" data-toggle="modal" data-target="#modal" id="add">
                    <div class="ht-35 justify-content-between">
                      ...
                      <span class="pd-15">Add New Committee</span>
                    </div>
                  </a>
                </div>
            </div>
              ...
              <div class="modal-content tx-size-sm">
                <div class="modal-body tx-center pd-y-20 pd-x-20">
                  ...
                  <div class="pd-x-30 pd-y-10">
                    <h5 class="tx-inverse  mg-b-30 model-title">Add Committee</h5>
                    <div class="form-group">
                       <input type="text" name="uid" id="uid" class="form-control pd-y-12" placeholder="Enter User Name">
                    </div><!-- form-group -->
                    <div class="form-group ">
                      <input type="text" name="crid" id="crid" class="form-control pd-y-12" placeholder="Enter committee role">
                    </div><!-- form-group -->
                    <div class="form-group ">
                      <div class="input-group">
                        <span class="input-group-addon"><i class="icon ion-ios-calendar tx-16 lh-0 op-6"></i></span>
                        <input type="date" id="startdate" class="form-control fc-datepicker" >
                      </div>
                    </div><!-- form-group -->
                    <div class="form-group ">
                      <div class="input-group">
                        <span class="input-group-addon"><i class="icon ion-ios-calendar tx-16 lh-0 op-6"></i></span>
                        <input type="date" id="enddate" class="form-control fc-datepicker" >
                      </div>
                    </div><!-- form-group --> 
            ...
            <table id="datatable1" class="table display responsive nowrap">
              <thead>
                <tr>
                  ...
                  <th class="wd-15p tx-center">Committee Role</th>
                  <th class="wd-25p tx-center">Start Date</th>
                  <th class="wd-20p tx-center">End Date</th>
    ...
    <script>
      ...
      $(document).ready(function(){ 
        $('#add').click(function(){  
           $('#user_form')[0].reset();  
           $('.model-title').text("Add Committee");  
           $('#action').val("Add");
           $('#op').val("Add");
      });  
       ...
       var dataTable =  $('#datatable1').DataTable({
          "ajax": {
            url : "<?php echo base_url(); ?>committee/displayCommittee",
            type : 'GET'
            },
          responsive: true,
          language: {
       ...
       $('.dataTables_length select').select2({ minimumResultsForSearch: Infinity });
        $(document).on('submit', '#user_form', function(event){  
           ...
           if(uid != '' && crid != '' && startdate != '' && enddate != '')  
           {  
                $.ajax({  
                     url:"<?php echo base_url() . 'committee/committee_action'?>",  
                     method:'POST',  
                     data:{uid:uid,crid:crid,startdate:startdate,enddate:enddate,cid:cid,op:op},   
        ...
        $(document).on('click', '.update', function(){  
           ...
           $.ajax({  
                url:"<?php echo base_url(); ?>committee/fetch_single_committee",  
                method:"POST",  
                data:{cid:cid},  
                dataType:"json",  
                success:function(data)  
                {  
                     ...
                     $('#modal').modal('show');  
                     $('#uid').val(data.u_id);  
                     $('#crid').val(data.committee_role_id); 
                     $('#startdate').val(data.start_date);  
                     $('#enddate').val(data.end_date);  
                     $('.model-title').text("Edit Committee");  
                     $('#cid').val(cid);    
                     $('#action').val("Edit"); 
                     $('#op').val("Edit");
        ...
        //Delete Meeting 
         $(document).on('click', '.delete', function(){  
           ...
           {  
                $.ajax({  
                     url:"<?php echo base_url(); ?>meetings/delete_single_meeting",  
                     method:"POST",  
                     data:{mid:mid},  
                     success:function(data)  
                     {  
