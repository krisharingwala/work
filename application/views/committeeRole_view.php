  ...
  <body>
                ...
                <div class="col-lg-8 col-md-12 d-flex justify-content-end">
                  <a href="#" class="btn btn-teal-info btn-with-icon" data-toggle="modal" data-target="#modal" id="add">
                    <div class="ht-35 justify-content-between">
                      ...
                      <span class="pd-15">Add New Committee Role</span>
                    </div>
                  </a>
                </div>
            </div>
              ...
              <div class="modal-content tx-size-sm">
                <div class="modal-body tx-center pd-y-20 pd-x-20">
                  ...
                  <div class="pd-x-30 pd-y-10">
                    <h5 class="tx-inverse  mg-b-30 model-title">Add Committee Role</h5>
                    <div class="form-group">
                       <input type="text" name="committee_role" id="committee_role" class="form-control pd-y-12" placeholder="Enter committee role">
                    </div><!-- form-group -->
    ...
    <script>
      ...
      $(document).ready(function(){ 
        $('#add').click(function(){  
           $('#user_form')[0].reset();  
           $('.model-title').text("Add Committee Role");  
           $('#action').val("Add");
           $('#op').val("Add");
      });  
       ...
       var dataTable =  $('#datatable1').DataTable({
          "ajax": {
            url : "<?php echo base_url(); ?>committeeRole/displayCommitteeRole",
            type : 'GET'
            },
          responsive: true,
          language: {
        ...
        $(document).on('submit', '#user_form', function(event){  
           ...
           var committee_role = $('#committee_role').val();  
           var crid = $('#crid').val();
           var op = $('#op').val();
           ...
           {  
                $.ajax({  
                     url:"<?php echo base_url() . 'committeeRole/committeeRole_action'?>",  
                     method:'POST',  
                     data:{committee_role:committee_role,crid:crid,op:op},   
        ...
        $(document).on('click', '.update', function(){  
           ...
           $.ajax({  
                url:"<?php echo base_url(); ?>committeeRole/fetch_single_committeeRole",  
                method:"POST",  
                data:{crid:crid},  
                dataType:"json",  
                success:function(data)  
                {  
                     ...
                     $('#modal').modal('show');  
                     $('#committee_role').val(data.committee_role);  
                     $('.model-title').text("Edit Committee Role");  
                     $('#crid').val(crid);    
                     $('#action').val("Edit"); 
                     $('#op').val("Edit");
