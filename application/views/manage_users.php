                ...
                <div class="col-lg-8 col-md-12 d-flex justify-content-end">
                  <a href="#" class="btn btn-teal-info btn-with-icon" data-toggle="modal" data-target="#modal" id="add">
                    <div class="ht-35 justify-content-between">
                      ...
                      <span class="pd-15">Add New Users</span>
                    </div>
                  </a>
                </div>
            </div>
              ...
              <div class="modal-content tx-size-sm">
                <div class="modal-body tx-center pd-y-20 pd-x-20">
                  ...
                  <div class="pd-x-30 pd-y-10">
                    <h5 class="tx-inverse  mg-b-30 model-title">Add Users</h5>
                  
                    <div class="form-group">
                       <input type="text" name="firstname" id="firstname" class="form-control pd-y-12" placeholder="Enter first name">
                    </div><!-- form-group -->
                    <div class="form-group">
                       <input type="text" name="middlename" id="middlename" class="form-control pd-y-12" placeholder="Enter middle name">
                    </div><!-- form-group -->
                    <div class="form-group">
                       <input type="text" name="lastname" id="lastname" class="form-control pd-y-12" placeholder="Enter last name">
                    </div><!-- form-group -->
                    <div class="form-group">
                       <input type="number" name="mobileno" id="mobileno" class="form-control pd-y-12" placeholder="Enter mobile number">
                    </div><!-- form-group -->
                    <div class="form-group">
                       <input type="email" name="email" id="email" class="form-control pd-y-12" placeholder="Enter email">
                    </div><!-- form-group -->
                    <div class="form-group">
                       <input type="password" name="password" id="password" class="form-control pd-y-12" placeholder="Enter password">
                    </div><!-- form-group -->
                   <div  class="form-group" class="dropdown">
                   <select id="u_type" class="form-control" name="u_type" data-placaeholdar="user type">
                   ...
                   <option value='a'> admin</option>
                    ...
                    <div class="form-group ">
                      <input type="text" name="flate_no" id="flate_no" class="form-control pd-y-12" placeholder="Enter flate no">
                    </div><!-- form-group -->
                    <div class="form-group ">
                      <input type="text" name="block_no" id="block_no" class="form-control pd-y-12" placeholder="Enter block no">
                    </div><!-- form-group -->
            ...
            <table id="datatable1" class="table display responsive nowrap">
              <thead>
                <tr>
                  ...
                  <th class="wd-5p tx-center">User Name</th>
                  <th class="wd-5p tx-center">Mobile Number</th>
                  <th class="wd-5p tx-center">Email</th>
                  <th class="wd-5p tx-center">User Type</th>                                                      
    ...
    <script>
      ...
      $(document).ready(function(){ 
       ...
       var dataTable =  $('#datatable1').DataTable({
          "ajax": {
            url : "<?php echo base_url(); ?>users/displayUsers",
            type : 'GET'
            },
          responsive: true,
          language: {
        ...
        $(document).on('submit', '#user_form', function(event){  
           ...
           var flate_no = $('#flate_no').val();
           var u_type = $('#u_type').val();
           var username=$('#block_no').val()+$('#flate_no').val()+$('#u_type').val();;
           var uid = $('#uid').val();
           var op = $('#op').val();
           if(firstname != '' && email != '' &&mobileno!='' && password != ''&& block_no != '' && flate_no != ''&&u_type!='')  
           {  
                $.ajax({  
                     url:"<?php echo base_url() . 'users/user_action'?>",  
                     method:'POST',  
                     data:{username:username,fname:firstname,mname:middlename,lname:lastname,mobilenum:mobileno,email:email,password:password,flate_no:flate_no,u_type:u_type,uid:uid,op:op},   
        ...
        $(document).on('click', '.update', function(){  
           ...
           $.ajax({  
             url:"<?php echo base_url(); ?>users/fetch_single_user",  
                method:"POST",  
                data:{uid:uid},  
                dataType:"json",  
                success:function(data)  
                {   
                    ...
                    $('#modal').modal('show');  
                     ...
                     $('#mobileno').val(data.mobilenum); 
                     $('#flate_no').val(data.flate_no);  
                    $('#u_type').val(data.u_type);  
                     // var bn=$('#block_no').val(data.username);
                      //alert(substr(bn,0,1));
                     $('.model-title').text("Edit User");  
                     $('#uid').val(uid);    
                     $('#action').val("Edit"); 
                     $('#op').val("Edit");
                }  
        ...
        $(document).on('click', '.delete', function(){  
           ...
           {  
                $.ajax({  
                     url:"<?php echo base_url(); ?>users/delete_single_user",  
                     method:"POST",  
                     data:{uid:uid},  
                     success:function(data)  
                     {  
